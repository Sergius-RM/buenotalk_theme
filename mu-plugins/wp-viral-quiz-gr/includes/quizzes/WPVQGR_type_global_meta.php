...
class WPVQGR_type_global_meta
{
		...
		$container = Container::make( 'post_meta', __("How to display the quiz?", 'wpvq') )
		    ...
		    ->add_fields( array(
				...
				Field::make( 'html', 'wpvq_shortcodeinfo_params_examples' )
    				->set_html( __('<p>Example :</p> <code>[wpViralQuiz id=' . (isset($_GET['post']) ? $_GET['post']:'XX') . ' columns=3]</code>', 'wpvq') ),
		    ));
		    ...
		    if (isset($_GET['post']))
		    {
			    $container->add_fields(array(
			   		Field::make( 'html', 'wpvq_shortcodeinfo_quickpost' )
	    				->set_html( __('<p>Create a new quiz-post <a href="post-new.php?wpvqgr_shortcode_id='.$_GET['post'].'">within one-click</a>.', 'wpvq') )
	    			));
			}
	...
	public function add_settings_marketing($array)
	{
		...
		(
            ...
            ->set_html('<div><a href="https://www.ohmyquiz.io/knowledgebase/configure-mailchimp-synchronisation/" target="_blank">Learn to configure Mailchimp</a></div>')
            ...
            ->set_html('
				<div id="wpvqgr-aweber-generate-auth">
					...
					<p>If you need help, <a href="https://www.ohmyquiz.io/knowledgebase/enable-aweber-synchronisation/" target="_blank">learn to configure Aweber</a></p>
				</div>')
            ...
            ->set_html('<div><a href="https://www.ohmyquiz.io/knowledgebase/configure-activecampaign-synchronisation/" target="_blank">Learn to configure ActiveCampaign</a></div>')
			...
			// Global thirdparty settings
			Field::make( 'checkbox', 'wpvqgr_settings_syncuser_thirdparty_saveresult', __('Save result in a RESULT field?', 'wpvq'))
			->set_help_text( __("Then create a custom field named <strong>RESULT</strong> in your autoresponder system.", 'wpvq') )
			->set_conditional_logic($thirdparty_conditions),
		);
